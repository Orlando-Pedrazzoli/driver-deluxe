...
function Boschcar({ MapsComponent }) {
  ...
  const getServices = async () => {
    try {
      const response = await getService('boschcarservice');
  ...
  return loading ? (
    ...
    <>
          ...
          <div className='flex flex-col px-6 py-8 space-y-6 rounded-sm sm:p-8 lg:p-12 lg:w-1/2 xl:w-2/5 bg-gray-200 text-gray-900'>
            <div className='flex space-x-2 sm:space-x-4 mt-8'>
              ...
              <div className='space-y-2'>
                <p className='text-lg font-medium leadi'>Bosch Car Service:</p>
                <p className='leadi'>
                  Bosch Car Service is the right choice for maintenance, repair,
                  and assembly, carried out by specialists. Bosch Car Service
                  workshops have the Bosch guarantee, the leader in supplying
                  original equipment for workshops. With over 13,000 certified
                  workshops, our network is one of the largest independent
                  chains in the world. Thanks to our qualification, we can take
                  care of all brands of motor vehicles.
                </p>
      ...
      <h1 className='text-center font-semibold text-2xl mb-6'>
        Check all <span className='font-bold'> BOSCH CAR SERVICES</span> on the
        map with
        <span className='text-orange-500 font-bold'> Orange ICON </span>
        <img
          src={orangeicon}
          alt='Icon'
